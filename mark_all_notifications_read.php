...
$stmt = $conn->prepare("UPDATE notifications SET is_read = ? WHERE admin_id = ?");
if (!$stmt) {
    header('Content-Type: application/json');
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => 'db_error'], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
...
$stmt->bind_param("ii", $is_read, $admin_id);
$result = $stmt->execute();
$stmt->close();

header('Content-Type: application/json; charset=utf-8');
echo json_encode(['success' => (bool)$result, 'is_read' => $is_read], JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
